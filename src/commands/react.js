  ...
  run: async (toolbox) => {
    ...
    await Promise.all([
      ...
      template.generate({
        ...
        target: `${name}/src/public/index.html`,
        props: { name },
      }),
      template.generate({
      ...
      installDependencies(),
      createComponent(`${name}/src/pages`, 'Main', name),
    ])

    info('Making first commit')
    run(`git --git-dir ${name}/.git --work-tree ${name} init && git --git-dir ${name}/.git --work-tree ${name} add . && git --git-dir ${name}/.git --work-tree ${name} commit -m 'Project initialized'`)
  },
}
